...
class HoePlan @PublishedApi internal constructor(
...
class HoeScope : ItemScope() {
    var attackDamage: Float = -1f
    var attackSpeed: Float = -2.0f
}

fun ItemRegistry.hoe(
    tier: Tier,
    config: HoeScope.() -> Unit = {},
) = bindName { rawName ->
    val name = rawName.toSnakeCase()
    val scope = HoeScope().apply(config)
    ...
    val plan = HoePlan(name, deferred, texture).apply {
        model = scope.model
